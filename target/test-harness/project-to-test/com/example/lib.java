...
class lib {
    ...
    static class StringMultimapHelper extends BaseAttributeHelper<Map<String, List<String>>> {

        StringMultimapHelper(String attributeName) {
            super(attributeName);
        }

        @Override
        public AttributeValue.Builder build(AttributeValue.Builder builder, Map<String, List<String>> value) {
            Map<String, AttributeValue> dynamoMap = new TreeMap<>();
            for (Map.Entry<String, List<String>> e : value.entrySet()) {
                List<AttributeValue> values = new ArrayList<>();
                for (String val : e.getValue()) {
                    values.add(AttributeValue.builder().s(val).build());
                }
                dynamoMap.put(e.getKey(), AttributeValue.builder().l(values).build());
            }
            return builder.m(dynamoMap);
        }

        @Override
        public Map<String, List<String>> extract(AttributeValue value) {
            Map<String, List<String>> returnMap = new HashMap<>();
            if (value.hasM()) {
                for (Map.Entry<String, AttributeValue> e : value.m().entrySet()) {
                    if (e.getValue().hasL()) {
                        returnMap.put(e.getKey(), e.getValue().l().stream().map(AttributeValue::s).toList());
                    } else {
                        returnMap.put(e.getKey(), List.of());
                    }
                }
            }
    ...
    public static class StringMapMutator<P> extends GenericMutator<Map<String, String>, P> {

        private final List<Map.Entry<String, String>> puts = new ArrayList<>();

        StringMapMutator(String attribute, P parent, AttributeHelper<Map<String, String>> helper) {
        ...
        void appendUpdateExpression(AtomicInteger counter, List<String> setExpressions, List<String> removeExpressions,
            ...
            for (Map.Entry<String, String> entry : puts) {
                ...
                setExpressions.add(String.format("#mm%d.#mk%d = :mmv%d", myCounter, itemCounter, itemCounter));
                expressionAttributeNames.put("#mm" + myCounter, attribute);
                expressionAttributeNames.put("#mk" + itemCounter, entry.getKey());
                expressionAttributeValues.put(":mmv" + itemCounter,
    ...
    public static class StringMultimapMutator<P> extends GenericMutator<Map<String, List<String>>, P> {
        ...
        void appendUpdateExpression(AtomicInteger counter, List<String> setExpressions, List<String> removeExpressions,
            ...
            for (Map.Entry<String, String> entry : adds) {
                ...
                setExpressions.add(
                        String.format("#smm%d.#smmk%d = list_append(if_not_exists(#smm%d.#smmk%d, :smme%d), :ssmv%d)",
                                myCounter, itemCounter, myCounter, itemCounter, emptyCounter, itemCounter));
    ...
    public static class GenericQuery<T, P> {
        ...
        public P lt(T value) {
            return singleValueComparison(ComparisonOperator.LT, value);
        }

        public P gt(T value) {
            return singleValueComparison(ComparisonOperator.GT, value);
        }
    ...
    /**
     * Consumes the input Stream and writes it to the ObjectOutputStream in chunks.
     * Protocol: [UTF ClassName] -> ([Int Size] [Obj...])* -> [Int -1]
     */
    static <T> void writeStream(java.io.ObjectOutputStream out, java.util.stream.Stream<T> inputStream, Class<T> type)
            throws java.io.IOException {
        out.writeUTF(type.getName());

        List<T> buffer = new ArrayList<>(50);
    ...
    private static <T> void writeChunk(java.io.ObjectOutputStream out, List<T> buffer) throws java.io.IOException {
    ...
    static <T> java.util.stream.Stream<T> readStream(java.io.ObjectInputStream in, Class<T> expectedType)
            throws java.io.IOException {

        String className = in.readUTF();
        if (!className.equals(expectedType.getName())) {
            throw new java.io.IOException(
                    "Type Mismatch! Expected: " + expectedType.getName() + ", Found: " + className);
        }

        Spliterator<T> spliterator = new Spliterators.AbstractSpliterator<T>(Long.MAX_VALUE,
                Spliterator.ORDERED | Spliterator.NONNULL) {

            int itemsRemainingInChunk = 0;
            ...
            public boolean tryAdvance(java.util.function.Consumer<? super T> action) {
                ...
                try {
                    if (itemsRemainingInChunk == 0) {
                        int nextChunkSize = in.readInt();

                        if (nextChunkSize == -1) {
                            sectionFinished = true;
                            return false; // End of this section
                        }

                        itemsRemainingInChunk = nextChunkSize;
                    }

                    Object obj = in.readObject();
                    action.accept(expectedType.cast(obj));
                    itemsRemainingInChunk--;
                    return true;

                } catch (java.io.IOException | ClassNotFoundException e) {
                    throw new RuntimeException("Error reading chunked stream", e);
                }
            }
        };
